...
export async function generateMetadata(props: {
  ...
  folderNameAndDescription = await prisma.accessToken
    .findUnique({
    ...
    .then((result) =>
      result
        ? { name: result.folder.name, description: result.folder.description }
        : null,
    );
...
export default async function FolderPage(props: {
  ...
  const searchParams = await props.searchParams;
  const params = await props.params;
  const hasAccess = await isAllowedToAccessFolder(
    params.folderId,
    searchParams.share,
    searchParams.h,
    searchParams.t,
  ...
  if (searchParams.share) {
    fetch(
      `${process.env.NEXT_PUBLIC_APP_URL}/api/tokens/increment?token=${searchParams.share}`,
    );
  }
  ...
  const filesWithSignedUrls = await Promise.all(
    folder.files.map(async (file) => ({
      ...file,
      signedUrl: await generateV4DownloadUrl(
      ...
      >
        <TokenProvider token={accessToken}>
          <FilesProvider
            ...
            defaultView={searchParams.view || ViewState.Grid}
          >
            <FolderContent />
          </FilesProvider>
        </TokenProvider>
