...
export default async function LocaleLayout(
	...
	const files = session?.user
		...
		: [];
	const accessTokens = session?.user
		...
		: [];
	const sharedWithMeFolders = session?.user
		? await AccessTokenService.getMultiple({
			...
			<SidebarProvider defaultOpen={!!session?.user}>
				...
				<SidebarInset className="flex-1 max-h-[calc(100svh-theme(spacing.4))]">
					...
					{session?.user?.emailVerified === false ? (
						<UnverifiedEmail
							locale={locale}
							userDeletionDate={addDays(session.user.createdAt, 7)}
						/>
					) : null}
