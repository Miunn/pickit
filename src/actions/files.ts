...
export async function initiateFileUpload(
  ...
  const folderUserId = await prisma.folder.findUnique({
    ...
    if (verificationData.type.startsWith("image/")) {
      ...
      const image = await prisma.file.create({
        data: {
          ...
          folderId: parentFolderId,
          createdById: folderUserId.createdById,
      ...
      try {
        const thumbnailBuffer =
          await extractThumbnailFromBuffer(uploadedBuffer);
        ...
        // Save thumbnail
        await GoogleBucket.file(
          `${folderUserId.createdById}/${parentFolderId}/${fileId}-thumbnail`,
      ...
      const mediainfo = await mediaInfoFactory({
        locateFile: (file) =>
          `${process.env.NEXT_PUBLIC_APP_URL}/mediainfo/${file}`,
      });
      ...
      const video = await prisma.file.create({
        data: {
          ...
          duration:
            metadata.media?.track.find(
              (track: Track) => track["@type"] === "General",
            )?.Duration || 0,
        },
        include: {
          tags: true,
...
export async function getImagesWithFolderAndCommentsFromFolder(
  folderId: string,
): Promise<{
...
export async function likeFile(
  ...
  if (user) {
    ...
    if (existingLike) {
      const like = await prisma.fileLike.delete({
...
export async function updateFilePosition(
  ...
  if (
    nextFile &&
    previousFile &&
    nextFile.position - previousFile.position < 2
  ) {
...
export async function deleteFile(
  ...
  if (file.type === FileType.VIDEO) {
    await GoogleBucket.file(
      `${file.createdById}/${file.folderId}/${file.thumbnail}`,
    ).delete();
  }

  await GoogleBucket.file(
    `${file.createdById}/${file.folderId}/${file.id}`,
  ).delete();
...
export async function processVideoAfterUpload(
  ...
  try {
    ...
    // Save thumbnail
    await GoogleBucket.file(`${user.id}/${folderId}/${video.thumbnail}`).save(
      thumbnailBuffer,
    );
...
async function extractThumbnailFromBuffer(
  videoBuffer: Buffer,
): Promise<Buffer> {
