...
export async function initiateFileUpload(
  ...
  try {
    await GoogleBucket.file(
      `${session.user.id}/${parentFolderId}/verification/${verificationToken}.json`
    ...
    const [uploadUrl] = await GoogleBucket.file(
      `${session.user.id}/${parentFolderId}/${fileId}`
    ).getSignedUrl({
      version: "v4",
      action: "write",
  ...
  try {
    ...
    if (verificationData.type.startsWith("image/")) {
      ...
      try {
        ...
        // Save thumbnail
        await GoogleBucket.file(
          `${session.user.id}/${parentFolderId}/${fileId}-thumbnail`
