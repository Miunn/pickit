...
export function E2EEncryptionProvider({ children }: { readonly children: React.ReactNode }) {
    ...
    const loadWrappingKeyFromSessionStorage = () => {
        ...
        if (wrappingKey) {
            crypto.subtle
                .importKey("jwk", JSON.parse(wrappingKey), { name: "AES-GCM" }, false, ["wrapKey", "unwrapKey"])
                .then(key => {
                    setWrappingKey(key);
                });
    ...
    return (
        <E2EEncryptionContext.Provider
            value={{ publicKey, wrappingKey, loadKeys, setupKeys, loadWrappingKeyFromSessionStorage }}
        >
            {children}
        </E2EEncryptionContext.Provider>
    );
