...
export const FilesProvider = ({
  ...
  const hasUserLikedFile = (fileId: string) => {
    ...
    const userAuthorized = file.likes.some(
      (like) => like.createdByEmail === user?.email,
    );
    ...
    const tokenAuthorized = file.likes.some(
      (like) => like.createdByEmail === token?.email,
    );
  ...
  const getSortedFiles = (
    ...
    if (sortStrategy !== "dragOrder") {
      const sortedItems = [
        ...getSortedImagesVideosContent(files, sortState),
      ] as ContextFile[];
      return sortedItems;
    }
