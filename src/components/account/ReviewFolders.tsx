		...
		<div className="h-full overflow-y-hidden">
			...
			<ScrollArea className="mt-6 h-full overflow-y-auto">
				...
				{folders.map(folder => (
					<div
						key={folder.id}
						className="flex items-center justify-between py-2 border-b border-gray-200 pr-px"
					>
						<div>
							<h4 className="font-semibold truncate">{folder.name}</h4>
							<p className="text-sm text-muted-foreground capitalize">
								{intlFormatter.dateTime(folder.createdAt, {
									weekday: "long",
									day: "numeric",
									year: "numeric",
									month: "long",
						...
						<div className="flex items-center gap-3">
							<p className="text-sm text-muted-foreground">
								{t("folder.images", { count: folder._count.files })}
							</p>
							<p className="text-sm text-muted-foreground">
							...
							<DropdownMenu>
								...
								<DropdownMenuContent align="end" className="min-w-40">
									...
									<DropdownMenuItem asChild>
										...
										>
											{t(
												"folder.dropdown.openFolder"
											)}
										</Link>
									</DropdownMenuItem>
									<DropdownMenuItem asChild>
										<Link
											href={`/${locale}/app/links`}
											className="cursor-default"
										>
											{t(
												"folder.dropdown.manageAccesses"
											)}
										</Link>
									</DropdownMenuItem>
									<DropdownMenuSeparator />
									...
									>
										{deleteFolderTranslations("trigger")}
									</DropdownMenuItem>
								</DropdownMenuContent>
							</DropdownMenu>
						</div>
