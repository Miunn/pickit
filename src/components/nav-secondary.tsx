                ...
                <SidebarMenu>
                    ...
                    <SidebarMenuItem>
                        <SidebarMenuButton asChild size="sm" className="items-start">
                            <Link href={`/app/account/billing`} className="h-auto flex flex-col items-start gap-1">
                                <p className="text-start">{t("used")}</p>
                                <div className="w-full flex items-center gap-2">
                                    <Progress
                                        value={(Number(userUsedStorage) / Number(userMaxStorage)) * 100}
                                        className="w-full"
                                    />
                                    <span className="text-nowrap">
                                        {formatBytes(Number(userUsedStorage))} / {formatBytes(Number(userMaxStorage))}
                                    </span>
                                </div>
                            </Link>
                        </SidebarMenuButton>
