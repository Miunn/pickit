  ...
  return (
    <>
      ...
      {selecting ? (
        <div
          className={
            "flex justify-between items-center mb-5 bg-gray-50 rounded-2xl w-full p-2"
          }
        >
                  ...
                  if (selecting) {
                    if (e?.shiftKey) {
                      if (!selected.includes(file.id)) {
                        const lastSelected = files.findIndex(
                          (f) => f.id === selected[selected.length - 1]
                        );
                        const currentSelected = files.findIndex(
                          (f) => f.id === file.id
                        );
                        const range = files.slice(
                          Math.min(lastSelected, currentSelected),
                          Math.max(lastSelected, currentSelected) + 1
                        );
                        setSelected([...selected, ...range.map((f) => f.id)]);
                        setSizeSelected(
                          sizeSelected +
                            range.reduce((acc, f) => acc + f.size, 0)
                        );
                      }
