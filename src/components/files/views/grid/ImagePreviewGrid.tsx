				...
				<ContextMenuContent>
					...
					<ContextMenuItem asChild>
						<a
							href={(() => {
								const url = new URL(
									`${process.env.NEXT_PUBLIC_APP_URL}/api/folders/${file.folderId}/${file.type === FileType.IMAGE ? "images" : "videos"}/${file.id}/download`
								);
								if (shareToken)
									url.searchParams.set("share", shareToken);
								if (shareHashPin)
									url.searchParams.set("h", shareHashPin);
						...
						<ContextMenuItem
							onClick={async () => {
								...
								if (r.error) {
									toast({
										title: t(
											"actions.setAsCover.errors.unknown.title"
										),
										description: t(
								...
								toast({
									...
									description: t(
										"actions.setAsCover.success.description"
									),
								});
							}}
						>
