...
import { cn, filterObjectOut, formatBytes } from "@/lib/utils";
...
export default function SelectingBar({
			...
			setFiles((prev: ContextFile[]) => {
				return prev.map(f =>
					selected.includes(f.id)
						? { ...f, tags: filterObjectOut(f.tags, "id", tag.id) }
						: f
				);
			});
		}
	...
	const handleTagUnselected = async (tag: FolderTag) => {
		setFiles(prev =>
			prev.map(f =>
				selected.includes(f.id) ? { ...f, tags: filterObjectOut(f.tags, "id", tag.id) } : f
			)
		);
