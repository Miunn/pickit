...
import RecentlyAdded from "@/components/files/views/grid/RecentlyAdded";
...
export const ImagesGrid = () => {
	...
	const handleDragEnd = async (event: DragEndEvent) => {
		...
		console.log("Active", activeIndex, sortedFiles[activeIndex]);
		console.log("Over", overIndex, sortedFiles[overIndex]);
		...
		if (overIndex === 0) {
			...
			newPosition = (await updateFilePosition(activeId, sortedFiles[sortedFiles.length - 1].id))
				.newPosition;
						...
						{sortedFiles.length === 0 ? (
							...
							sortedFiles.map((file, idx) => (
								<ImagePreviewGrid
									...
									onSelect={() =>
										handleSelectFile(file.id, file.size)
									}
								/>
							))
						)}
