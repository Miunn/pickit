...
export const PoiWindowContent = ({
  file,
  onClose,
}: ClusterWindowContentProps) => {
      ...
      <div className="bg-white border border-primary rounded-lg overflow-hidden shadow-lg mb-[23px] max-w-64">
        ...
        <div className="p-2">
          <h2 className="w-fit hover:underline text-sm text-gray-500 font-normal flex justify-between items-center gap-2">
            <Link
              href={`/app/folders/${file.folderId}?${searchParams.toString()}`}
            >
              {file.folder.name}
            </Link>
          </h2>
          <h3 className="text-sm font-bold flex justify-between items-center gap-2">
            {file.name}
          </h3>
        </div>
      </div>
