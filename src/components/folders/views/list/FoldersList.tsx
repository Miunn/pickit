...
export default function FoldersList({
	...
	readonly folders: (FolderWithLastSlug &
		FolderWithAccessToken &
		FolderWithFilesCount &
		FolderWithCover &
		FolderWithFilesWithFolderAndComments)[];
	...
	return (
		<>
			{Object.keys(rowSelection).length > 0 ? (
				<div
					className={
						"flex justify-between items-center mb-5 bg-accent rounded-2xl w-full p-2"
					}
				>
					<div className={"flex gap-2 items-center"}>
						...
						<h2>
							...
							{formatBytes(
								folders
									.filter(i =>
										Object.keys(rowSelection).includes(i.id)
									)
			...
			<Table className="w-full">
				...
				<TableBody>
					{table.getRowModel().rows?.length ? (
						...
						<TableRow>
							<TableCell
								colSpan={foldersListViewColumns.length}
