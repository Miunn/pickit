...
export default function BillingDashboard() {
    ...
    const { data: history } = useBillingHistorySWR();
            ...
            <div className="flex flex-col gap-4">
                <div className="grid grid-cols-3 gap-4">
                    <Card className="flex flex-col justify-between">
                        <CardHeader>
                            <CardTitle>
                                {t("storage.using", { used: formattedUsedStorage, max: formattedMaxStorage })}
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="w-full">
                            <div className="w-full flex items-center gap-2">
                                ...
                                <span className="text-nowrap text-sm text-muted-foreground">
                                    {formattedUsedStorage} / {formattedMaxStorage} ({usedStoragePercentage.toFixed(2)}%)
                                </span>
                            </div>
                        </CardContent>
                    </Card>
                    <Card className="flex flex-col justify-between">
                        <CardHeader>
                            <CardTitle>
                                {t("albums.using", { used: user?._count.folders, max: user?.maxAlbums })}
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="w-full">
                            <div className="w-full flex items-center gap-2">
                                <Progress
                                    value={user?._count.folders ? (user?._count.folders / user?.maxAlbums) * 100 : 0}
                                    className="w-full"
                                />
                                <span className="text-nowrap text-sm text-muted-foreground">
                                    {user?._count.folders} / {user?.maxAlbums}
                                </span>
                            </div>
                        </CardContent>
                    </Card>
                    <Card className="flex flex-col justify-between">
                        <CardHeader>
                            <CardTitle>
                                {t("sharing.using", { used: user?._count.folders, max: user?.maxSharingLinks })}
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="w-full">
                            <div className="w-full flex items-center gap-2">
                                <Progress
                                    value={
                                        user?._count.folders ? (user?._count.folders / user?.maxSharingLinks) * 100 : 0
                                    }
                                    className="w-full"
                                />
                                <span className="text-nowrap text-sm text-muted-foreground">
                                    {user?._count.folders} / {user?.maxSharingLinks}
                                </span>
                            </div>
                        </CardContent>
                    </Card>
