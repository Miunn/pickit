...
const PlanCard = ({
    ...
    return (
        <Label
            htmlFor={plan}
            className={cn("relative flex items-center gap-3 border p-3 rounded-lg", selected && "border-primary")}
        >
            {isCurrentPlan && (
                ...
                <CardContent className="w-full flex flex-col">
                    ...
                    {currentPlan === Plan.FREE ? (
                        ...
                        <Button
                            ...
                            onClick={async () => {
                                setChangePlan(true);
                                const { status, nextAmount } = await getPreviewInvoiceBeforeUpdate(
                                    getPriceId(selectedPlan, isYearly ? "yearly" : "monthly")
                                );
                                setChangePlan(false);
                                if (status === "error") {
