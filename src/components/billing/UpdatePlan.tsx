...
const PlanCard = ({
	...
	return (
		<Label
			htmlFor={plan}
			className={cn(
				"relative flex items-center gap-3 border p-3 rounded-lg",
		...
		>
			...
			<div className="flex flex-col gap-2">
				...
				<p className="text-xl">
					{currency}{" "}
					{isYearly
						? Number(price.yearly / 12).toFixed(2)
						: Number(price.monthly).toFixed(2)}
				...
				<CardContent className="w-full flex flex-col">
					<div className="flex items-center gap-2 mb-1 self-end">
						...
						<Label htmlFor="isYearly">
							{isYearly ? t("upgrade.yearly") : t("upgrade.monthly")}
						</Label>
					</div>
					<RadioGroup
						value={selectedPlan}
					...
					{currentPlan === Plan.FREE ? (
						<CheckoutDialog plan={selectedPlan}>
							<Button
								...
								disabled={selectedPlan === currentPlan || false}
							>
								{t("upgrade.button")}
							</Button>
						</CheckoutDialog>
					...
					{selectedPlan === Plan.FREE && currentPlan !== Plan.FREE ? (
						...
						<Button
							className="w-full text-center mt-2"
							disabled={selectedPlan === currentPlan || changePlan}
							onClick={async () => {
								setChangePlan(true);
								const { status, nextAmount } =
									await getPreviewInvoiceBeforeUpdate(
										getPriceId(
											selectedPlan,
