...
const EmailInput = ({
    ...
    return (
        <>
            ...
            />
            {error && (
                <div id="email-errors" className="text-destructive text-sm mt-1">
                    {error}
                </div>
            )}
        </>
...
export default function CheckoutForm() {
    ...
    return (
        <form onSubmit={handleSubmit} className="border border-gray-200 rounded-lg p-4 space-y-2">
            <EmailInput email={email} setEmail={setEmail} error={emailError} setError={setEmailError} disabled={true} />
            <PaymentElement
                id="payment-element"
                options={{
                    layout: {
            ...
            <Button disabled={isLoading} id="submit" className="w-full my-3">
                {isLoading ? <Loader2 className="size-4 animate-spin" /> : `Pay ${checkout.total.total.amount} now`}
            </Button>
            {/* Show any error or success messages */}
            {message && (
                <div id="payment-message" className="text-destructive text-sm">
                    {message}
                </div>
            )}
        </form>
