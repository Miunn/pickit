...
export default function ResetPasswordForm({ locale, token, status }: { locale: string, token: string, status: PasswordResetRequestStatus }) {
    ...
    const renderSuccess = () => (
        <div className="w-fit flex flex-col items-center gap-2">
            ...
            <Button variant={"link"} asChild><Link href={"/signin"}>{t('success.actionLong')}</Link></Button>
        </div>
    );
